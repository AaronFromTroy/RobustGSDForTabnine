...
async function checkPackageContents() {
  ...
  for (const pattern of sensitivePatterns) {
    if (stdout.toLowerCase().includes(pattern)) {
      throw new Error(\`❌ SECURITY: Package may contain sensitive file: \${pattern}\`);
    }
  }
  ...
  if (foundDev.length > 0) {
    throw new Error(\`❌ DEV FILES: Package contains dev files: \${foundDev.join(', ')}\`);
  }

  console.log('✅ No sensitive or dev files detected\n');
}
...
async function checkPackageSize() {
  ...
  try {
    ...
    if (sizeBytes > 10 * 1024 * 1024) { // 10MB
      throw new Error(\`❌ PACKAGE TOO LARGE: \${sizeMB}MB exceeds 10MB limit\`);
    }

    if (sizeBytes > 5 * 1024 * 1024) { // 5MB
      console.warn(\`⚠️  WARNING: Package is large (\${sizeMB}MB). Consider excluding unnecessary files.\n\`);
    } else {
      console.log(\`✅ Package size acceptable (\${sizeMB}MB)\n\`);
    }
  } catch (err) {
    if (err.code === 'ENOENT') {
...
async function checkRequiredFiles() {
  ...
  for (const file of required) {
    ...
    try {
      ...
      throw new Error(\`❌ MISSING: Required file \${file} not found\`);
    }
  }
...
async function validateExports() {
  console.log('🔗 Validating exports field...\n');
  ...
  for (const [key, value] of Object.entries(pkg.exports)) {
    ...
    try {
      ...
      throw new Error(\`❌ BROKEN EXPORT: "\${key}" points to non-existent file: \${value}\`);
    }
  }
