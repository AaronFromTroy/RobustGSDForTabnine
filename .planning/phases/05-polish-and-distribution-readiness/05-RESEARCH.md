...
├── .npmignore           # Exclude dev files (or use files field)
├── LICENSE              # Required: MIT/Apache 2.0
├── README.md            # Primary documentation
├── QUICKSTART.md        # 5-minute getting started
├── CHANGELOG.md         # Auto-generated by semantic-release
├── CONTRIBUTING.md      # Contributor guidelines
├── scripts/             # Core library code
│   └── *.js            # ESM modules (type: "module")
├── templates/           # Template files to distribute
├── guidelines/          # Guideline files to distribute
...
**How to avoid:**
1. Go to npmjs.com → package settings → Trusted Publishers → Add GitHub Actions
...
async function checkPackageContents() {
  ...
  for (const pattern of sensitivePatterns) {
    if (stdout.includes(pattern)) {
      throw new Error(`❌ SECURITY: Package contains sensitive file: ${pattern}`);
    }
  }
...
- 🐛 Report bugs via [GitHub Issues](https://github.com/username/gsd-for-tabnine/issues)
