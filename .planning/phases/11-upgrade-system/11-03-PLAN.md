...
- For each migration:
  - Logs: "🔧 [description] ([version])"
  - Dynamically imports migration.implementation file
  - Calls default export: await migrationFn.default()
  - On success: logs "   ✅ Complete"
  - On failure: logs "   ❌ Failed: [error]", throws error to stop upgrade
...
import('./scripts/migration-runner.js').then(async (m) => {
  ...
  console.log('✅ Migration runner works');
});
"
```
