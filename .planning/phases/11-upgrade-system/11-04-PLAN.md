...
- Steps:
  1. **Smart Source Detection** (if source not specified):
     a. Log "🔍 Detecting upgrade source..."
     b. Check npm availability using version-checker.checkNpmAvailability()
     c. If npm available: source = 'npm', log "✅ npm registry available"
     d. If npm unavailable:
        - Log "⚠️ npm registry unavailable: ${reason}"
        - Call detectLocalSource()
        - If local found: source = 'local', localPath = detected, log "✅ Found local source"
...
```
🔍 Detecting upgrade source...
⚠️  npm registry unavailable: Network error
🔍 Checking for local upgrade sources...
✅ Found local source: ../gsd-upgrade
📁 Using local source: ../gsd-upgrade
```
