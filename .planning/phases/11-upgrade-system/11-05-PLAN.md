...
<task type="auto">
  ...
  <action>
Add Test Suite 17 to gsd/scripts/integration-test.js with 12 tests for upgrade system validation.
...
async function testSuite17_UpgradeSystem() {
  ...
  // Test 3: checkForUpdates returns correct structure
  try {
  ...
  // Test 4: backup-manager module exports
  try {
  ...
  // Test 5: createBackup creates valid backup (offline test)
  try {
  ...
  // Test 6: file-merger module exports
  try {
  ...
  // Test 7: determineFileStrategy returns correct strategies
  try {
  ...
  // Test 8: migration-runner module exports
  try {
  ...
  // Test 9: getApplicableMigrations with empty registry
  try {
  ...
  // Test 10: upgrade-manager module exports
  try {
  ...
  // Test 11: previewUpgrade returns correct structure
  try {
  ...
  // Test 12: trigger-detector detects upgrade trigger
  try {
...
Expected: 12 tests for upgrade system modules and integration.
  </action>
  <verify>
```bash
...
Expected: All tests pass (95 existing + 12 new = 107 tests total).
  </verify>
  <done>Test Suite 17 added with 12 tests, all tests pass, integration-test.js updated</done>
</task>
...
### From Source (Copied Installation)

If you copied the `gsd/` directory into your project:

1. **Check for updates:**
   ```bash
   # In your project directory
   cd gsd
   npm install -g gsd-for-tabnine@latest
   ```

2. **Run upgrade with dry-run preview:**
...
3. **Apply upgrade:**
   Say "upgrade GSD" and confirm, or run:
   ```bash
...
2. 12 upgrade system tests implemented
3. All tests pass (107 total)
4. CHANGELOG.md created with upgrade documentation
5. Version history documented
6. Migration notes template provided
7. README.md updated with upgrade section
8. Safety features documented
9. Rollback instructions provided
10. Documentation follows industry standards (Keep a Changelog, semver)
</success_criteria>
